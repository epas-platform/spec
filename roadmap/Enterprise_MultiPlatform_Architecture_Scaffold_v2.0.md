...
5. **Five-Plane Architecture** – v2.0 adds a **Contract Plane** alongside Control, Data, Event, and Observability. Contracts are not events; the two planes have distinct substrates and retention profiles.
...
**v2.0 Preserves Every v1.3 Principle.** No v1.3 principle is removed or weakened. v2.0 is additive.
...
- **v2.0.0** – AI-first documentation, SDK-first architecture, contract-based trust model with external anchoring, five-plane model (adds Contract Plane), agentic governance, edge deployment class, NATS JetStream primary, DID-based identity and delegation. First major version bump under SemVer: this release introduces changes that mark previously-conformant v1.3 patterns (direct API consumption by clients, bearer-token authority passthrough, mutations without contracts, events without `contractid`) as non-conformant. See [ADR 001](./docs/adr/001-semver-v2.md) for the version-label correction (drafted under the working title "v1.4" in `1.4 Planning/` before SemVer alignment).
...
- v2.0 is vendor-neutral. Reference implementations inform the specification but are not named in specification text.

## Conformance Declaration

Every service or platform claiming EPAS v2.0 conformance MUST publish a `conformance.yaml` at the repository root declaring its conformance state. Partial conformance during migration from v1.3 is permitted and MUST be explicitly declared. A template is published at [2.0/conformance.yaml.template](./2.0/conformance.yaml.template).

⸻

# Document Set Map

EPAS v2.0 publishes as a **document set** under `2.0/`. This scaffold file provides narrative, amendments to sections not broken out as standalone specs, and stakeholder cross-references. Each document in the set is normative and retrievable on its own.

| # | Document | Scope |
|---|----------|-------|
| [00](./2.0/00-overview.md) | v2.0 Overview | Strategic positioning, 8 shifts, migration plan, stakeholder guide |
| [01](./2.0/01-core-principles-and-tenets.md) | Core Principles and Architectural Tenets | 15 principles, machine-readable tenet list, five-plane model |
| [02](./2.0/02-sdk-first-architecture.md) | SDK-First Architecture | Normative SDK specification; `ContractHandle` as primary return type |
| [03](./2.0/03-contract-based-trust-model.md) | Contract-Based Trust Model | Contract lifecycle, envelope schema, three-concern ledger decomposition (modeling / substrate / external anchoring) |
| [04](./2.0/04-api-architecture.md) | API Architecture (Command, Query, Internal) | REST / GraphQL / gRPC taxonomy, canonical URL structure, task dispatch under contract |
| [05](./2.0/05-documentation-for-machine-readers.md) | Documentation for Machine Readers | `llms.txt`, `CLAUDE.md` required sections, five RAG-optimization rules, MCP tool documentation |
| [06](./2.0/06-agentic-governance.md) | Agentic Governance and Guardrails | Three-tier classification, four enforcement layers, emergency override, trust accumulation |
| [07](./2.0/07-edge-nats-flutter.md) | Edge, NATS, and Flutter Clients | Edge Node deployment class, NATS leaf-node topology, Go aggregator, Flutter FSM requirement |
| [08](./2.0/08-event-driven-architecture.md) | Event-Driven Architecture | NATS JetStream primary, Kafka/Redpanda permitted with justification, CloudEvents `contractid` extension, AsyncAPI mandatory |
| [09](./2.0/09-identity-and-delegation.md) | Identity, Delegation, and Cryptographic Authority | DID-based identity, four identity classes, three-tier agent model, signed delegation chains, per-request signatures |

**How to read v2.0:**

- **Executives**: Read the "What's New" section above, then [00 Overview](./2.0/00-overview.md).
- **Architects**: Read [01 Core Principles](./2.0/01-core-principles-and-tenets.md), then [02](./2.0/02-sdk-first-architecture.md), [03](./2.0/03-contract-based-trust-model.md), and [04](./2.0/04-api-architecture.md) in sequence.
- **Security**: Read [03 Contract-Based Trust Model](./2.0/03-contract-based-trust-model.md), [06 Agentic Governance](./2.0/06-agentic-governance.md), and [09 Identity and Delegation](./2.0/09-identity-and-delegation.md).
- **Platform engineers**: Read [02 SDK-First](./2.0/02-sdk-first-architecture.md), [04 API Architecture](./2.0/04-api-architecture.md), [05 Documentation](./2.0/05-documentation-for-machine-readers.md).
- **Edge / mobile engineers**: Read [07 Edge, NATS, and Flutter Clients](./2.0/07-edge-nats-flutter.md) after the architectural tentpoles.

⸻

# Core Principles

Normative specification: [2.0/01-core-principles-and-tenets.md](./2.0/01-core-principles-and-tenets.md)

v2.0 defines 15 core principles: the 10 carried forward from v1.3 and 5 new principles that enable the AI-first and SDK-first shift. Principles are listed in precedence order; earlier principles win conflicts.
...
See [01 Core Principles and Tenets](./2.0/01-core-principles-and-tenets.md) for the full normative text and the complete machine-readable tenet list.
...
Normative specification: [2.0/01-core-principles-and-tenets.md § 6](./2.0/01-core-principles-and-tenets.md#6-five-plane-architecture)

v1.3 defined four planes: Control, Data, Event, Observability. v2.0 adds a fifth: the **Contract Plane**. Contracts are distinct from events: contracts have low volume and long retention; events have high volume and operational retention. Systems that conflate the two planes fail under audit.
...
Normative specification: [2.0/01-core-principles-and-tenets.md § 4](./2.0/01-core-principles-and-tenets.md#4-architectural-tenets-v14).

v2.0 defines ~35 tenets grouped as follows:
...
Every tenet is enforced at one or more of: compile-time, CI-time, deploy-time, runtime. Tenets enforced only by documentation or developer discipline are non-conformant. See the full tenet list and enforcement layer mapping in [01 § 4–5](./2.0/01-core-principles-and-tenets.md#4-architectural-tenets-v14).
...
## v2.0 Amendment — Delegation Chains and DID-Based Identity

v2.0 replaces token-passthrough authentication with cryptographically provable identity and delegation. The authoritative specification is [2.0/09-identity-and-delegation.md](./2.0/09-identity-and-delegation.md).
...
See [09 Identity and Delegation](./2.0/09-identity-and-delegation.md) for the full envelope schema, chain verification rules, and revocation semantics.
...
v2.0 does not amend the cost-tracking mechanics. The v2.0 contract model does contribute new attribution possibilities — every cost event can now be attributed to the contract under which it was incurred — but this attribution is additive and does not change the v1.3 cost tracking architecture.
...
v2.0 amends only the approval authority: fine-tuned model deployments now require a governance-owner signed policy change per [06 Agentic Governance § 5](./2.0/06-agentic-governance.md#5-declarative-classification) when the deployment affects agent behavior. The v1.3 CAIO approval model remains authoritative for model selection; the v2.0 amendment applies only when the model is consumed by an autonomous agent.

⸻

# Section 11. Event-Driven Architecture

Superseded by [2.0/08-event-driven-architecture.md](./2.0/08-event-driven-architecture.md).
...
The v1.3 Phase 1 / Phase 2 phased rollout language is retired. Teams select a backbone on merit against the criteria in [08 § 4](./2.0/08-event-driven-architecture.md#4-event-bus-selection-criteria).
...
v2.0 amendment: the Event Plane emits metrics (per-subject message rate, consumer lag, DLQ depth, retry rate, schema validation failures, edge buffer depth) that feed the Observability Plane. See [08 § 14](./2.0/08-event-driven-architecture.md#14-observability-of-the-event-plane).
...
- **EU AI Act high-risk governance** — high-risk AI system governance requirements (human oversight, bias testing, logging) map to v2.0's Agentic Governance (specification 06). Approval-required operations are the human oversight mechanism; contract-ledger refusal records provide the bias-relevant audit trail.
...
v2.0 amendment — new subsection **15.x Agentic Security & Guardrails**, superseded by the standalone specification [2.0/06-agentic-governance.md](./2.0/06-agentic-governance.md).
...
v2.0 amendment: security incidents involving contract-ledger tampering, agentic guardrail violations, or delegation-chain compromise are automatically P0 regardless of scope.
...
v2.0 amendment: the contract ledger and its external anchoring have DR requirements distinct from operational data. External anchoring is not a DR substitute for the ledger — the platform must recover its own ledger substrate from DR backups, and the external anchor provides integrity verification that the DR-restored ledger matches the pre-incident state.
...
v2.0 amendment: the Contract Plane's Record-stage latency (5–50ms for CT-style logs; 10–100ms for immutable event stores; 50–500ms for witness networks) contributes to end-to-end command acceptance latency. Platforms SHOULD select ledger substrate based on the tenant tier's latency budget.
...
v2.0 amendment: agentic governance policy changes are change-managed as **normal changes** (or **emergency changes** with post-hoc review) and produce contract-ledger entries per [06 § 5.2](./2.0/06-agentic-governance.md#52-policy-versioning).
...
├── conformance.yaml               # EPAS v2.0 conformance declaration (REQUIRED)
...
v2.0 introduces substantial additions on top of v1.3. The following phasing is recommended for new and migrating deployments.
...
- Update `architectural_tenets` configuration to the v2.0 list.
...
- Publish EPAS v2.0 conformance attestation.

⸻

# Summary

v2.0 is not an incremental revision. It is the framework-level acknowledgment that AI agents are first-class platform consumers, and that their consumption must be governed by cryptographically provable consent. Everything else — SDK-first, machine-readable documentation, five-plane architecture, external anchoring, edge deployment, DID-based identity — derives from that acknowledgment.

v2.0 is also additive: nothing in v1.3 is removed or weakened. A v1.3-conformant platform can migrate to v2.0 incrementally without abandoning operational properties that are already working.

Reference implementations may publish their own mapping documents that trace v2.0 requirements to specific products, technologies, and naming conventions. Such mappings are out of scope for this specification. v2.0 remains vendor-neutral.
...
Superseded by [2.0/08-event-driven-architecture.md § 4](./2.0/08-event-driven-architecture.md#4-event-bus-selection-criteria) (selection criteria) and § 5–6 (recommended primary and permitted alternatives).
...
Carried forward from v1.3. v2.0 additions:

- v2.0-D1: SDK-first is mandatory; direct API consumption by clients is non-conformant.
- v2.0-D2: Contract ledger is mandatory for mutating operations.
- v2.0-D3: Contract ledger MUST be externally anchored; public blockchain anchoring is the recommended external medium.
- v2.0-D4: Storing full contracts on a public blockchain (as opposed to anchoring Merkle roots) is non-conformant.
- v2.0-D5: NATS JetStream is the recommended primary event bus; Kafka/Redpanda are permitted with declared justification.
- v2.0-D6: Refusal is a first-class contract outcome, represented as a structured signed object, never as an exception.
- v2.0-D7: Bearer tokens prove identity only; authority is always expressed through signed delegation.

## Appendix F. Documentation Checklist (NEW in v2.0)

Every EPAS v2.0 repository MUST satisfy the following checklist. CI validates the checklist on every pull request.
...
See [2.0/conformance.yaml.template](./2.0/conformance.yaml.template) for the machine-readable template. Every repository claiming EPAS v2.0 conformance publishes a completed `conformance.yaml` at the repository root.

⸻

**Enterprise Multi‑Platform Architecture Scaffold v2.0.0 — Draft**
